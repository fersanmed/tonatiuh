...
To model the input aperture of the central recevier do the following:
  ...
  1. Double click on the newly created TSeparatorKit node and change its name to "Target\_Frame".
  1. With the "Target\_Frame" node selected, go to the "Transform" properties dialog, just below the tree view, and set the transform propoerties as follows:
```
...
center             0 0 0
```
  1. With the TSeparatorKit "Target\_Frame" selected in the Tree View, go to the plug-ins Menu Bar and click on the TShapeKit icon to create a TShapeKit node under the "Target\_Frame" node.
  1. By default this newly created TShapeKit will be labeled "TShapeKit1", double click on the name and change it to "Target\_ShapeKit".
  1. With the "Target\_ShapeKit" node selected, go to the plug-ins Menu Bar and click on the "!Flat\_rectangle" TShapeNode icon to add a TShapeNode to the "Target\_ShapeKit".
  1. By default this newly created TShapeNode will be labeled "Flat\_Rectangle", keep the name.
  1. Click on the "Flat\_Rectangle" TShapeNode to select it. Go to the “ShapeFlatRectangle” dialog that will appear below the Tree View and set the values of the parameters that define this shape node to:
```
width      10
height     10
activeSide FRONT
```
  1. Under the "Target\_Frame" TSeparatorKit click the TShapeKit node labeled "Target\_ShapeKit" to select it.
  1. With the "Target\_ShapeKit" selected, go to the Tonatiuh plug-ins Menu Bar and select the Specular\_Standard\_Material icon.
